...
The pilot uses a fixed deterministic target of 10,000 users stratified equally across the four oracle clusters. The target was raised after cluster-independent support probes found 1,261 eligible items for a 1,250-user draw and 1,987 eligible items for a 2,500-user draw at the 20-positive threshold. The 1,250-user result is recorded in `results/explorations/support_probe_1250.json`; the 2,500-user result is recorded in `results/explorations/raw_snapshot_preflight.json`. User eligibility and item support are computed from pilot training data only. Each cluster receives its own catalog containing items with at least 20 training interactions in that cluster. The all cluster intersection is retained as a secondary catalog.
